...
function checkCanonicalSurface(checks, profile) {
  ...
  if (profile.tunnelTarget === 'shadcn') {
    const canonical = run('npm', ['run', 'mcp:shadcn-context:doctor'], { timeout: 120000 })
    check(
      checks,
      `profile:${profile.id}:canonical-surface`,
      canonical.status === 0 ? 'ok' : 'error',
      canonical.status === 0 ? 'schema-bound shadcn workbench tools OK' : firstLine(canonical.stderr) || firstLine(canonical.stdout) || 'shadcn workbench doctor failed'
    )
  }
