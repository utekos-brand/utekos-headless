...
/** @type {string[]} */
const targets =
  process.argv.slice(2).length ? process.argv.slice(2).map(p => join(process.cwd(), p)) : walkMd(ROOT)
...
console.log(
  `OK: ${targets.length === 1 && statSync(targets[0]).isDirectory() ? walkMd(targets[0]).length : targets.filter(t => t.endsWith('.md')).length || walkMd(ROOT).length} file(s) compliant`
)
