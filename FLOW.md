...
| Klarna-kandidat       | `codex/release-klarna-product-cards` flytter Express Checkout til tilgjengelige produktkort og bruker én delt, idempotent SDK-loader per dokument. En kontrollert browser-smoke med lokal SDK-stub viste tre knapper ved både 390 px og 1440 px, én `<script>`-node og tre container-loads. Dette beviser vår rendring og single-loader-kontrakt, ikke Klarna-providerens aksept. Reell preview-smoke er blokkert til `NEXT_PUBLIC_KLARNA_CLIENT_ID` er verifisert som ikke-tom for Preview/Production og de aktuelle originene er tillatt hos Klarna. Vercel-build feiler lukket dersom identifikatoren mangler. |
...
| Lokal integrasjonsaudit | Git-operasjoner, Microsoft Merchant, PostHog SDK, Klarna-produktkort, storefront-tilgjengelighet, MCP-/driftsverktøy og kildehygiene ble kombinert fra `origin/main` uten mergekonflikter. Frossen install uten supply-chain-bypass, route-typegenerering, 14/14 målrettede tester, MCP build med 52 servere, MCP/commerce doctor, lint av alle endrede kodefiler, TypeScript og Vercel-lignende build med 96/96 ruter var grønne. Den midlertidige auditreferansen ble fjernet etter dokumentasjon og ble aldri pushet eller deployet. |
