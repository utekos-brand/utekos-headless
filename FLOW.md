...
- Provider dead-letter-registeret ble gjenåpnet 2026-07-10 av 48
  nye Google-avvisninger. Alle 48 skyldes at Measurement Protocol
  mottok `page_location` over 100 tegn. Lokal retting fjerner
  query/hash og utelater fortsatt overlang verdi, men er ikke
  produksjonsdeployet. Ingen replay skal utføres før den rettede
  payloaden er deployet og nye events er verifisert grønne.
...
2026-07-10 med lokale read-only-kommandoer, MCP/plugin-lesing,
Vercel-logger og Supabase-rapporter.
...
| Produksjonsdeploy     | Vercel inspect/list 2026-07-08 viser production deployment `utekos-headless-55g9vsbve-utekos-marketing-group.vercel.app`, opprettet 2026-07-08T18:55:14.894Z, `READY`, target `production`.                                                                                                                                                                                                                                                                                                                                                                        |
...
| Replay route          | Produksjonscronen kalte `/api/cron/replay-dead-letter` hvert 15. minutt mens `DEAD_LETTER_REPLAY_ENABLED` var av; ruten returnerte derfor forventet `403`. Lokal konfigurasjon fjerner cronplanen og beholder ruten som secret- og godkjenningsgated manuell engangskjøring. Produksjonsstøyen stopper først etter godkjent deploy.                                                                                                                                                                                                                                    |
...
| Provider health       | `npm run ops:provider-dispatch-report -- --fail-on-alerts` 2026-07-10T17:22Z returnerte bevisst exit 2: 13 215 provider-rader, 3 aktive kø-rader, 48 failed/dead-lettered Google-rader, 48 uløste dead letters og 2 kritiske alerts. Meta og Microsoft UET hadde 0 nye failed/dead-lettered rows.                                                                                                                                                                                                                                                                            |
| Dead letters          | De 382 historiske radene ble klassifisert/resolved 2026-07-08 uten replay. Registeret ble gjenåpnet av 48 nye Google `ga_error`-rader; alle dokumenterer `page_location` over Measurement Protocol-grensen på 100 tegn. Read-only replay-plan viser 48 kandidater, men ingen replay er utført. Lokal payloadretting må deployes og observeres før en separat godkjent replay vurderes.                                                                                                                                                                                    |
...
| 4. Provider dispatch | Meta/Google har store mengder `succeeded`; retry/dead-letter-ruter finnes, og 382 historiske rader er klassifisert/lukket uten blind replay. | 48 nye Google-rader er uløste fordi browser-fallback sendte `page_location` over Measurement Protocol-grensen. Lokal retting er ikke deployet. | Google mister disse eventene, og rapportgaten er kritisk rød til produksjonsrettingen er bevist. | Deploy payloadrettingen, bevis at nye Google-events ikke får samme feil, og vurder deretter separat godkjent replay med original idempotency. |
...
| --------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| P0        | Nye Google dead letters                 | 48 uløste Google-rader 2026-07-10; alle er `ga_error` for `page_location` over 100 tegn. Lokal payloadretting er grønn, men ikke deployet. | Godkjenn og deploy den scoped rettingen; observer nye events; kjør rapporten grønn før separat godkjenning av eventuell replay | 0 nye feil med samme grunn, 0 failed/dead-lettered og 0 unresolved etter godkjent håndtering |
...
| P1        | Identifier coverage                     | Google dead letters domineres av manglende `client_id`; Microsoft purchase mangler fortsatt attribution/ids i nyeste rad | Mål coverage for `client_id`, `fbp`, `fbc`, `msclkid` per steg                                                                              | Coverage-rapport per eventtype og consent state                                                          |
...
- De 382 radene som ble klassifisert og lukket 2026-07-08 er
  fortsatt historisk lukket uten provider replay. Påstanden om at
  dead-letter-registeret ikke er et aktivt P0-gap ble derimot
  foreldet 2026-07-10 da 48 nye Google `page_location`-feil kom inn.
  Replay-ruten forblir secret- og godkjenningsgated og skal ikke
  ligge i Vercels tilbakevendende cronplan.
