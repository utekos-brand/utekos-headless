...
| Klarna-kandidat       | `codex/release-klarna-product-cards` flytter Express Checkout til tilgjengelige produktkort og bruker én delt, idempotent SDK-loader per dokument. Kontrollert lokal browser-smoke viser synlige knapper og korrekt variantbytte ved 390 px og 1440 px. Read-only Vercel CLI bekrefter at `NEXT_PUBLIC_KLARNA_CLIENT_ID` finnes som sensitiv variabel for både Preview og Production uten at verdien ble eksponert. Dette beviser ikke Klarna-providerens aksept; allowed origins og synlig knapp må fortsatt verifiseres i Git-triggered Preview. |
