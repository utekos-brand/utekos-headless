...
2026-07-18T20:51Z var 205 providerbekreftet `SUCCESS`, mens 151 var
...
| Provider health       | Fersk Supabase-kontroll 2026-07-18T20:51Z viser 0 failed/dead-lettered og 0 uløste Google/Meta dead letters. Data Manager-statuscronen har flyttet 205 utførte requests til `provider_confirmed_success`; 151 er ikke-terminale statuskandidater, hvorav 111 sist var `PROCESSING`. Dette er ikke dispatch-feil. |
...
| 5. Shopify purchase  | En ekte, samtykket Meta-klikkreise har bevist standard Shopify checkout → betalt Purchase med identisk `external_id`, `_fbp`, `_fbc` og `fbclid` i checkout, Shopify-attributter og webhook-event. Meta mottok eventet på første forsøk. | Den konkrete Google-requesten er fortsatt `PROCESSING`, betalt Klarna Express er ikke observert, og Microsoft UET CAPI-adapter er ikke aktiv. | Purchase-events er de viktigste signalene for budoptimalisering og kapitalallokering. | Observer Google `SUCCESS`, en separat betalt Klarna-reise og reintroduser/verifiser Microsoft UET CAPI før full tre-provider-dekning hevdes. |
...
- "Supabase checkout snapshot-fallback er ikke live før Vercel deploy" er
  ...
  mottak; Google har lagret request-ID og er fortsatt ikke-terminal
  `PROCESSING`.
...
- Google Data Manager `SUCCESS` for Purchase-requesten til ordre `1866`.
