...
| `purchase`            | active         | Intended: Shopify orders-paid; verified active: browser purchase + Meta backfill | intended webhook `/api/shopify/webhooks/orders-paid`; verified browser/backfill | `purchase`; no browser dataLayer owner | `purchase` G                         | `Purchase` M             | `purchase` MS-S active                | order/financial state; transaction ID | operational ledger; provider consent from checkout attribution |
| `refund`              | active         | Intended: Shopify refunds-create; production webhook delivery unverified         | intended webhook `/api/shopify/webhooks/refunds-create`                         | `refund`; no browser dataLayer owner   | `refund` G                           | -                        | -                                     | refund ID                             | operational ledger; Google requires analytics                  |
