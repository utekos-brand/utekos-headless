...
| META-009 | Meta browser/server dedupe is not proven; ViewContent Deduplication not set up | Verified fail | Events Manager UI + Graph + Test Events | 2026-07-20: ViewContent «not set up»; other events still parsing; Graph omits `dedupe_key_feedback`; Test Events pair used different event_ids (DEV-020) |
| MS-001 | Published UET tag is `97247724` | Verified | Published GTM | CDN payload |
| MS-002 | Microsoft UET CAPI purchase worker is registered | Verified (code) | Repository | `microsoft_uet:purchase` in adapter/worker registries; catalog `serverOutbox: active`; production smoke pending |
| MS-003 | Non-purchase Microsoft server outbox remains blocked | Verified | Repository | `page_view` / `view_item` / cart funnel still `blocked_no_worker` |
