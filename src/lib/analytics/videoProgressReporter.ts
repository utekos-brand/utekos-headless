'use client'

// Reporter exported for video milestone wiring; no video controller is active yet.
import { sendGTMEvent } from '@next/third-parties/google'
import { readBrowserReporterContext } from './browserReporterContext'
import { browserPageViewSession } from './pageViewSession'
import {
  buildVideoProgressDataLayerEvent,
  createCanonicalVideoProgress,
  type CanonicalVideoProgress,
  type CanonicalVideoProgressCustomData
} from './videoProgressEvent'
import { startVideoProgressCollectorTransport } from './videoProgressCollectorTransport'

export type ReportCanonicalVideoProgressInput = {
  customData: CanonicalVideoProgressCustomData
  pageViewId?: string
}

export function reportCanonicalVideoProgress(
  input: ReportCanonicalVideoProgressInput
): () => void {
  if (typeof window === 'undefined') {
    return () => {}
  }

  try {
    const clientContext = readBrowserReporterContext()
    const pageView = browserPageViewSession.ensure({
      pageUrl: clientContext.pageUrl,
      ...(clientContext.documentReferrer ?
        { documentReferrer: clientContext.documentReferrer }
      : {})
    })

    const event = createCanonicalVideoProgress({
      environment: clientContext.environment,
      eventId: globalThis.crypto.randomUUID(),
      eventTime: new Date().toISOString(),
      pageUrl: clientContext.pageUrl,
      pageTitle: clientContext.pageTitle,
      pageViewId: input.pageViewId ?? pageView.pageViewId,
      ...(pageView.referrerUrl ?
        { referrerUrl: pageView.referrerUrl }
      : {}),
      consent: clientContext.consent,
      customData: input.customData,
      ...(clientContext.browserId ?
        { browserId: clientContext.browserId }
      : {}),
      ...(clientContext.clickId ?
        { clickId: clientContext.clickId }
      : {}),
      eventDeviceInfo: clientContext.eventDeviceInfo
    })

    sendGTMEvent(buildVideoProgressDataLayerEvent(event))
    return startVideoProgressCollectorTransport(event)
  } catch (error) {
    queueMicrotask(() => {
      throw error
    })
    return () => {}
  }
}

export type { CanonicalVideoProgress }
