...
import {
  buildSortApplyDataLayerEvent,
  createCanonicalSortApply,
  type CanonicalSortApply,
  type CanonicalSortApplyCustomData
} from './sortApplyEvent'
import { startSortApplyCollectorTransport } from './sortApplyCollectorTransport'

export type ReportCanonicalSortApplyInput = {
  customData: CanonicalSortApplyCustomData
  pageViewId?: string
}

export function reportCanonicalSortApply(
  input: ReportCanonicalSortApplyInput
): () => void {
  ...
  try {
    ...
    const event = createCanonicalSortApply({
    ...
    sendGTMEvent(buildSortApplyDataLayerEvent(event))
    return startSortApplyCollectorTransport(event)
  } catch (error) {
    queueMicrotask(() => {
      throw error
    })
...
export type { CanonicalSortApply }
