...
export type CanonicalFilterApplyCustomData = z.infer<
  typeof canonicalFilterApplyCustomDataSchema
>
...
type CreateCanonicalFilterApplyInput = {
...
export type FilterApplyDataLayerEvent = {
  event: 'filter_apply'
  event_id: string
  event_time: string
  source: 'web'
  page_view_id?: string
  custom_data: CanonicalFilterApplyCustomData
  canonical_event: CanonicalFilterApply
}

export function createCanonicalFilterApply(
  input: CreateCanonicalFilterApplyInput
): CanonicalFilterApply {
  const eventDeviceInfo = mapEventDeviceInfo(input.eventDeviceInfo)

  return canonicalFilterApplySchema.parse({
    schema_version: 1,
...
export function buildFilterApplyDataLayerEvent(
  event: CanonicalFilterApply
): FilterApplyDataLayerEvent {
  return {
    event: 'filter_apply',
