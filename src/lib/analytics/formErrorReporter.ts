...
import {
  buildFormErrorDataLayerEvent,
  createCanonicalFormError,
  type CanonicalFormError,
  type CanonicalFormErrorCustomData
} from './formErrorEvent'
import { startFormErrorCollectorTransport } from './formErrorCollectorTransport'

export type ReportCanonicalFormErrorInput = {
  customData: CanonicalFormErrorCustomData
  pageViewId?: string
}

export function reportCanonicalFormError(
  input: ReportCanonicalFormErrorInput
): () => void {
  ...
  try {
    ...
    const event = createCanonicalFormError({
    ...
    sendGTMEvent(buildFormErrorDataLayerEvent(event))
    return startFormErrorCollectorTransport(event)
  } catch (error) {
    queueMicrotask(() => {
      throw error
    })
...
export type { CanonicalFormError }
