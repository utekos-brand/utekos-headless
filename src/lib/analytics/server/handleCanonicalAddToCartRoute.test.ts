...
test('accepted add-to-cart returns 202 without scheduling outbox drain', async () => {
  let runBatchCalled = false
  let scheduleAfterCalled = false
  const expectedResponse = new Response(null, { status: 202 })
  const request = new Request('https://utekos.no/api/events/add-to-cart', {
    method: 'POST'
  })
...
test('duplicate add-to-cart returns 200 without scheduling outbox drain', async () => {
  let runBatchCalled = false
  let scheduleAfterCalled = false
...
test('collect error propagates without scheduling outbox drain', async () => {
  ...
  await assert.rejects(
    handleCanonicalAddToCartRoute(request, {
      collect: async () => {
        throw expectedError
      },
      runBatch: async () => {
