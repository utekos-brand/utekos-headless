...
import {
  ...
  type GoogleDataManagerViewItemOutboxDatabase,
  type RawGoogleDataManagerViewItemAttempt
...
const executePostgresQuery: GoogleDataManagerViewItemOutboxQueryExecutor =
...
const DEAD_LETTER_QUERY = `
  ...
  insert into ops.dead_letter_events (source, reason, payload, metadata)
  select
    'google',
    $4,
...
function assertCompleted(rows: QueryRow[], attemptId: string) {
  if (rows.length !== 1) {
    throw new Error(`Google outbox attempt ${attemptId} is no longer processing`)
  }
}

function parseClaimedRow(
  row: QueryRow | undefined
): RawGoogleDataManagerViewItemAttempt | null {
  if (!row) return null
  const attemptId = row.attempt_id
  const attemptCount = row.attempt_count

  if (typeof attemptId !== 'string' || typeof attemptCount !== 'number') {
    throw new Error('Invalid Google outbox claim result')
  }
...
): GoogleDataManagerViewItemOutboxDatabase {
  return {
    claimNext: async () => {
      const rows = await executeQuery(CLAIM_NEXT_QUERY, ['google', 'view_item'])
    ...
    markDeadLettered: async outcome => {
      const reason =
        ...
        : 'google_data_manager_permanent_error'
