...
export type MicrosoftUetCapiTokenEnvKey = (typeof MICROSOFT_UET_CAPI_TOKEN_ENV_KEYS)[number]
 ...
 * Source: Campaign Management `GetUetTagAuthKey` via OAuth (runtime refresh),
 * or env aliases below as fallback.
 * Not interchangeable with Microsoft Ads OAuth (`MICROSOFT_ADS_ACCESS_TOKEN`)
 * or the Ads API developer token (`MICROSOFT_ADS_DEVELOPER_TOKEN`).
 */
...
export function getMicrosoftUetCapiTokenEnvPresence(
  ...
  return Object.fromEntries(
    MICROSOFT_UET_CAPI_TOKEN_ENV_KEYS.map(key => [key, Boolean(env[key]?.trim())])
