...
export function prepareCaptureData(body: CaptureBody, context: CaptureContext): CheckoutAttribution {
  const { cookies, clientIp, userAgent, consentProvenance } = context
  const { services } = consentProvenance
  ...
  const userData: ExtendedUserData = {
    ...
    ...(services.microsoftAdvertising ? { msclkid: cookies.msclkid || undefined } : {})
  }
