  ...
  return (
    <SkreddersyMotionProvider>
      <article
        ...
        className='bg-background w-full bg-background text-foreground'
      >
        <m.div
        ...
        >
          ...
          <m.p
            className='leading-text-paragraph text-foreground/90 mx-auto max-w-2xl font-sans text-balance text-foreground/90'
            variants={revealItem}
          >
        ...
        <div className='flex flex-col pb-20 xl:hidden'>
          {Steps.map((step, index) => (
            ...
            >
              <m.div
                className='border-foreground/10  relative aspect-square w-full overflow-hidden border-y border-foreground/10 bg-card'
                variants={revealScale}
              >
                <Image
                  src={step.image}
              ...
              <div className='mx-auto w-full max-w-xl px-6 pt-6 md:px-8'>
                <m.div
                  className='text-primary mb-2 flex items-center gap-2 text-primary'
                  variants={revealItemLeft}
                >
                ...
                <m.p
                  className='leading-text-paragraph text-foreground/85 mt-2 font-sans tracking-wide text-foreground/85 md:text-2xl'
                  variants={revealItem}
                >
                  {step.description}
                </m.p>
        ...
        <div className='hidden w-full xl:flex'>
          <div className='border-foreground/10  sticky top-0 flex h-screen w-1/2 items-center justify-center overflow-hidden border-r border-foreground/10 bg-card'>
            {Steps.map((step, index) => (
              <m.div
                key={step.id}
                className={cn(
              ...
              >
                <div className='border-foreground/15 bg-background/40 relative aspect-square w-[min(86%,82vh)] overflow-hidden rounded-3xl border border-foreground/15 bg-background/40 shadow-2xl'>
                  <Image
                    src={step.image}
                    alt={step.title}
                    fill
                  ...
                  />
                  <div className='from-background absolute inset-x-0 bottom-0 h-28 bg-linear-to-t from-background to-transparent opacity-60' />
                </div>
              </m.div>
            ))}
              ...
              {Steps.map((_, i) => (
                <div
                  ...
                  className={cn(
                    ...
                    activeStep === i ?
                      'bg-accent w-8 bg-accent shadow-[0_0_8px_var(--color-accent)]'
                    : 'bg-foreground/30 w-2 bg-foreground/30'
                  )}
                />
              ))}
            </div>
          ...
          <div className='bg-background flex w-1/2 flex-col bg-background'>
            {Steps.map((step, index) => (
              <m.div
                key={step.id}
                className='border-foreground/10 flex min-h-screen flex-col justify-center border-b border-foreground/10 px-20 last:border-0'
                initial='hidden'
                whileInView='visible'
                viewport={{
                  ...skreddersyViewport,
              ...
              >
                <div>
                  <m.div
                    className='text-primary mb-4 flex items-center gap-3 font-sans text-base leading-4 font-medium text-primary md:text-lg'
                    variants={revealItemLeft}
                  >
                    <m.span
                      className='text-primary inline-flex shrink-0 text-primary'
                      variants={revealPop}
                    >
                      {step.icon}
                    </m.span>
                  ...
                  <m.p
                    className='leading-text-paragraph text-foreground/85 max-w-lg font-sans text-base text-foreground/85 md:text-2xl'
                    variants={revealItem}
                  >
                    {step.description}
                  </m.p>
