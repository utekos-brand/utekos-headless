...
export function ConclusionSection() {
  return (
    <article className={`${compareModelsTheme.lightSection} py-20 sm:py-28`}>
      <div className='mx-auto max-w-7xl px-[6vw]'>
        <div className='grid gap-12 lg:grid-cols-[0.45fr_0.55fr] lg:items-start'>
          <div>
            ...
            <p
              className={`leading-text-paragraph mt-6 text-lg sm:text-xl ${compareModelsTheme.bodyMuted}`}
            >
          ...
          <div data-nosnippet className={compareModelsTheme.listDivider}>
            {faqItems.map(item => (
              <details key={item.question} className='group py-6'>
                <summary className='cursor-pointer list-none font-sans text-xl leading-[1.05] font-bold tracking-[-0.01em] text-foreground marker:hidden'>
                  <span className='inline-flex w-full items-center justify-between gap-6'>
                    {item.question}
                    <span
                      className={`text-2xl leading-none transition-transform duration-300 group-open:rotate-45 ${compareModelsTheme.bodyMuted}`}
                    >
                      +
                    </span>
                  </span>
                ...
                <p
                  data-nosnippet
                  className={`leading-text-paragraph mt-4 max-w-2xl text-base ${compareModelsTheme.bodyMuted}`}
                >
                  {item.answer}
                </p>
              </details>
