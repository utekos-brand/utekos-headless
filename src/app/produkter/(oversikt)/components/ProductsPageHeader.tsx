  ...
  return (
    <header className='relative mb-12 w-full overflow-hidden rounded-3xl bg-[radial-gradient(circle_at_top,color-mix(in_oklab,var(--ancient-water)_16%,transparent),transparent_42%),linear-gradient(180deg,color-mix(in_oklab,var(--havdyp)_90%,black)_0%,color-mix(in_oklab,var(--background)_94%,black)_100%)] pt-12 pb-16'>
      <div className='pointer-events-none absolute inset-0 -z-10 overflow-hidden'>
        ...
        <div className='absolute top-0 left-1/2 h-[800px] w-[1000px] -translate-x-1/2 opacity-25 mix-blend-screen'>
          <div
        ...
        <div className='via-cloud-dancer/30 absolute inset-x-10 top-0 h-px bg-linear-to-r from-transparent to-transparent' />

        {STAR_POSITIONS.map((position, i) => (
          <div
            key={i}
            className='motion-star bg-cloud-dancer absolute h-1 w-1 animate-pulse rounded-full opacity-16'
            style={{
              top: `${position.top}%`,
              left: `${position.left}%`,
      ...
      <div className='relative z-10 container mx-auto px-4 text-center'>
        <div className='motion-badge mb-8 inline-flex items-center justify-center'>
          <BrandBadge
            ...
            className='group border-cloud-dancer/18 relative border px-5 py-2.5 shadow-[0_14px_32px_-24px_rgba(8,10,24,0.62)] sm:px-6 sm:py-3'
          >
            <span className='absolute inset-0 overflow-hidden rounded-full'>
              <span className='via-cloud-dancer/25 absolute top-0 -left-full size-full skew-x-12 bg-linear-to-r from-transparent to-transparent transition-all duration-1000 group-hover:left-full' />
            </span>
            <UtekosWordmark
        ...
        <p className='motion-desc /95 mx-auto mt-8 max-w-2xl text-lg leading-normal text-foreground/95 md:text-xl'>
          ...
          <span className='relative inline-block font-medium text-foreground'>
            redefinert
            <span className='motion-underline dark:bg-dark-primary absolute bottom-0 left-0 h-[2px] w-full origin-left bg-primary' />
          </span>{' '}
