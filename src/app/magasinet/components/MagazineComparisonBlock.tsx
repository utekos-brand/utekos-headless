      ...
      <div className='mt-8 grid grid-cols-1 gap-4 lg:grid-cols-3'>
        {block.columns.map(column => (
          ...
          >
            ...
            {column.text && (
              <p className='text-background/74 mt-3 text-base leading-[1.55] text-background/74'>
                {column.text}
              </p>
            )}
            <ul className='mt-5 space-y-3'>
              {column.items.map(item => (
                <li
                  key={item}
                  className='leading-text-paragraph text-background/80 flex gap-3 text-sm text-background/80'
                >
