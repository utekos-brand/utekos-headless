...
import { handleCanonicalSelectPromotionRequest } from '@/lib/analytics/server/handleCanonicalSelectPromotionRequest'
import { handleCanonicalSelectPromotionRoute } from '@/lib/analytics/server/handleCanonicalSelectPromotionRoute'
import { postgresCanonicalEventStore } from '@/lib/analytics/server/postgresCanonicalPageViewStore'

export const maxDuration = 60

export function POST(request: Request) {
  return handleCanonicalSelectPromotionRoute(request, {
    collect: currentRequest =>
      handleCanonicalSelectPromotionRequest(currentRequest, {
