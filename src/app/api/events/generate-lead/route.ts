...
import { handleCanonicalGenerateLeadRequest } from '@/lib/analytics/server/handleCanonicalGenerateLeadRequest'
import { handleCanonicalGenerateLeadRoute } from '@/lib/analytics/server/handleCanonicalGenerateLeadRoute'
import { postgresCanonicalEventStore } from '@/lib/analytics/server/postgresCanonicalPageViewStore'

export const maxDuration = 60

export function POST(request: Request) {
  return handleCanonicalGenerateLeadRoute(request, {
    collect: currentRequest =>
      handleCanonicalGenerateLeadRequest(currentRequest, {
