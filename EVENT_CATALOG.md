...
5. Provider-routeren kan bare opprette server-outbox-rader når mappingen har
   `serverOutbox: active`.
6. De eneste aktive server-outbox-kombinasjonene i v1 er
   `google:view_item` og `meta:view_item`.
...
| `view_item_list` | planned | Produktliste når navngitt liste og løste items faktisk er synlige | `page_view_id + item_list_id + impression_sequence`; ny synlig sekvens | C2 | R30 |
| `select_item` | planned | Produktlenken når et akseptert valg starter navigasjon | `interaction_id`; ny akseptert interaksjon | C2 | R30 |
...
| `add_to_wishlist` | planned | Wishlist-store etter bekreftet persistens | `wishlist_mutation_id`; ny vellykket mutasjon | C3 | R90 |
| `add_to_cart` | planned | Shopify cart-service etter akseptert mutasjon og oppdatert cart med linjen | `cart_mutation_id`; ny vellykket Shopify-mutasjon | C3 | R90 |
| `remove_from_cart` | planned | Shopify cart-service etter akseptert fjerning og oppdatert cart | `cart_mutation_id`; ny vellykket fjerning | C3 | R90 |
| `view_cart` | planned | Cart-side eller drawer når løst cart-innhold faktisk er synlig | `page_view_id + cart_id + view_sequence`; ny synlig sekvens | C2 | R30 |
| `begin_checkout` | planned | Shopify checkout-service etter gyldig checkout-token eller URL | `checkout_id + creation_revision`; ny checkout | C3 | R90 |
...
| `purchase` | planned | Verifisert Shopify order-paid webhook etter relevant financial state | `order_id + financial_state`; ny relevant state-overgang | C4 | R7Y |
| `refund` | planned | Verifisert Shopify refund webhook etter opprettet refund | `refund_id`; ny Shopify-refund | C4 | R7Y |
| `search` | planned | Search-controller etter eksplisitt søk og løst resultat | `search_id`; nytt eksplisitt søk | C2 | R30 |
| `view_search_results` | planned | Search-results når løst resultatrevisjon faktisk er synlig | `search_id + result_revision`; ny synlig revisjon | C2 | R30 |
| `view_promotion` | planned | Promotion-observer ved minst 50 % synlighet i ett sammenhengende sekund | `page_view_id + promotion_id + impression_sequence`; ny kvalifiserende sekvens | C2 | R30 |
| `select_promotion` | planned | Promotion-lenke når akseptert valg starter handling eller navigasjon | `interaction_id`; ny akseptert interaksjon | C2 | R30 |
| `generate_lead` | planned | Lead-service etter akseptert og lagret innsending | `submission_id`; ny akseptert lead | C5 | R25M |
| `form_start` | planned | Form-controller ved første meningsfulle verdiendring, aldri fokus alene | `form_id + page_view_id`; ny sidevisning | C2 | R30 |
| `form_submit` | planned | Form-service etter serverakseptert innsending | `submission_id`; ny akseptert innsending | C5 | R25M |
| `form_error` | planned | Form-service når definitiv validerings- eller serverfeil vises | `attempt_id`; nytt feilet forsøk | C6 | R90 |
| `filter_apply` | planned | Produktfilter etter at oppdatert resultat er committet | `interaction_id + result_revision`; ny committet interaksjon | C2 | R30 |
| `sort_apply` | planned | Produktsortering etter at sortert resultat er committet | `interaction_id + result_revision`; ny committet interaksjon | C2 | R30 |
| `variant_select` | planned | Variant-controller etter at valgt variant er løst og committet | `interaction_id + variant_id`; nytt committet valg | C2 | R30 |
| `size_guide_view` | planned | Størrelsesguide når guideflaten faktisk er synlig | `page_view_id + guide_id + open_sequence`; ny synlig åpning | C2 | R30 |
...
| `scroll_depth` | planned | Scroll-observer ved første passering av 25/50/75/90 % per sidevisning | `page_view_id + threshold`; ny side eller terskel | C2 | R30 |
| `video_progress` | planned | Video-controller ved første passering av 10/25/50/75/90/100 % | `page_view_id + video_id + milestone`; ny side, video eller milepæl | C2 | R30 |
